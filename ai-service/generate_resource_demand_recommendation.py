...
# Latest resource demand prediction
result = (
    supabase
    ...
    .eq("target_entity", "hospital_resource_demand")
...
if not result.data:
    raise Exception("No resource demand prediction found.")
...
print(f"Target   : {prediction['target_date']}")
print(f"Demand   : {prediction['prediction_value']}")
print(f"Label    : {prediction['prediction_label']}")
print(f"Risk     : {prediction['risk_level']}")

# Generate recommendation
rpc_result = supabase.rpc(
    "generate_resource_demand_recommendation",
    {
